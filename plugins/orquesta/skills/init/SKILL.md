...
Cerrá en dos líneas, **sin correr ningún script vos** (lanzar `pwsh` desde acá dispara un prompt de permiso de Claude Code por "shell anidado" que no aporta nada): resumí qué quedó escrito, y decile que el siguiente paso es `/orquesta:doctor` — siempre después de init, nunca antes — para validar el entorno con esa config, y `/orquesta:estado` si quiere ver la tabla de enrutamiento con la fuente de cada valor. Los dos corren sin pedir permiso.
